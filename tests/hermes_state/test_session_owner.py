...
def test_owner_filter_shows_only_own_hides_untagged_and_others(db):
    ...
    # STRICT ownership: only the caller's own sessions. Untagged (legacy /
    # non-interactive) rows AND other owners' rows are hidden under "My".
    assert ids == {"mine"}
