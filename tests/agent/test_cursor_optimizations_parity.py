...
def test_parity_persist_bounded_scan():
    ...
    # The fork's flush backfills a wall-clock "timestamp" onto unstamped rows
    # (see run_agent timestamp backfill, #ACP replay). Freeze time.time so the
    # A/B agents stamp identical values and the parity compare stays exact.
