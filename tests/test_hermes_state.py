...
class TestListSessionsRich:
    ...
    def test_last_active_tracks_assistant_and_tool_messages(self, db):
        import time
        db.create_session("s1", "cli")
        db.append_message("s1", "user", "Hello")
        sessions = db.list_sessions_rich()
        user_ts = sessions[0]["last_active"]
        assert user_ts is not None
        time.sleep(0.01)
        db.append_message("s1", "assistant", "Hi there!")
        db.append_message("s1", "tool", "tool output")
        sessions = db.list_sessions_rich()
        # Agent output counts as activity: last_active advances past the user
        # message so the ACP session list can resort on turn end.
