...
from hermes_constants import ACP_OWNER_NOTE_PREFIX, strip_owner_note
...
# Multimodal messages persist ``content`` with the ``\x00json:`` sentinel
# prefix, and sqlite's text SUBSTR/LENGTH stop at the leading NUL — a plain
# text slice of such a row yields the empty string. Reading through a BLOB cast
# keeps the whole value addressable (it arrives in Python as ``bytes``, which
# ``_shape_preview`` decodes).
_PREVIEW_CONTENT_SQL = "CAST(REPLACE(REPLACE(m.content, X'0A', ' '), X'0D', ' ') AS BLOB)"
_PREVIEW_SCAFFOLDED_SQL = f"m.content LIKE '{SKILL_SCAFFOLD_SQL_LIKE}'"
# ``\x00json:`` as a BLOB literal — the LIKE operator can't express a leading
# NUL, so sentinel detection compares the first six bytes instead.
_PREVIEW_MULTIMODAL_SQL = (
    "SUBSTR(CAST(m.content AS BLOB), 1, 6) = CAST(X'006A736F6E3A' AS BLOB)"
)
# An owned ACP session's first user message opens with the authenticated-owner
# note (~200 chars of identity line plus parenthetical), which the head window
# would cut mid-note — leaving ``strip_owner_note`` unable to find its end, so
# the preview leaks note text. Such rows get the wider window.
_PREVIEW_OWNER_NOTE_SQL = f"m.content LIKE '{ACP_OWNER_NOTE_PREFIX}%'"
...
# around SKILL_EXCERPT_JOINT. Multimodal and owner-note rows also need the wider
# slice, since their display text sits behind an envelope/preamble that the head
# window would cut.
_PREVIEW_RAW_SELECT = (
    f"CAST(CASE WHEN {_PREVIEW_SCAFFOLDED_SQL}"
    ...
    f" WHEN {_PREVIEW_MULTIMODAL_SQL} OR {_PREVIEW_OWNER_NOTE_SQL}"
    f" THEN SUBSTR({_PREVIEW_CONTENT_SQL}, 1, {_PREVIEW_SCAFFOLD_WINDOW * 2})"
    f" ELSE SUBSTR({_PREVIEW_CONTENT_SQL}, 1, {_PREVIEW_HEAD_CHARS}) END AS BLOB)"
)

_PREVIEW_CONTENT_JSON_PREFIX = "\x00json:"
# Salvage path for a multimodal slice cut mid-envelope: the first text part.
_PREVIEW_TEXT_PART_RE = re.compile(r'"text"\s*:\s*"((?:[^"\\]|\\.)*)')


def _flatten_preview_raw(raw: Any) -> str:
    """Decode a raw preview slice into display text.

    The SELECT hands back ``bytes`` (see ``_PREVIEW_CONTENT_SQL``). Multimodal
    content is a JSON list of parts behind the ``\\x00json:`` sentinel, so peel
    the sentinel and flatten the text parts — otherwise the preview shows the
    ``[{'type': 'text', ...}]`` repr instead of the user's words. Mirrors the
    flattening in :meth:`SessionDB.get_session_messages_preview`.
    """
    if isinstance(raw, (bytes, bytearray)):
        raw = bytes(raw).decode("utf-8", "replace")
    text_raw = str(raw or "")
    # Peel the sentinel (full, or a NUL-stripped ``json:`` remnant from
    # older/alternate SELECT paths) before decoding.
    was_json = True
    if text_raw.startswith(_PREVIEW_CONTENT_JSON_PREFIX):
        text_raw = text_raw[len(_PREVIEW_CONTENT_JSON_PREFIX):]
    elif text_raw.startswith("json:"):
        text_raw = text_raw[len("json:"):]
    else:
        was_json = False
    decoded: Any = text_raw
    stripped = text_raw.lstrip()
    if stripped.startswith("["):
        # The slice may cut a multimodal list mid-part; retry against a
        # trimmed-to-last-complete-object candidate.
    ...
    if isinstance(decoded, str):
        if was_json:
            # The sentinel says this is a JSON envelope that still failed to
            # decode (slice cut mid-string): salvage the first text part rather
            # than leaking the raw envelope. Plain text that merely starts with
            # "[" never reaches here — it carries no sentinel.
            match = _PREVIEW_TEXT_PART_RE.search(decoded)
            if match:
                try:
...
def _shape_preview(raw: Any) -> str:
    ...
    text = _flatten_preview_raw(raw).strip()
    if not text:
        return ""
