...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # Combine: upstream binds the caller's ContextVars into the worker
    # (_context_thread_target); the fork additionally re-applies the
    # threading.local session context so log records carry the [session_id]
    # tag. Both are required — hermes_logging._session_context is a
    # threading.local, which contextvars.copy_context() does NOT carry.
    _sid = getattr(agent, "session_id", None)
    t = threading.Thread(
        target=_context_thread_target(
            lambda: run_with_session_context(_sid, _call)
        ),
        daemon=True,
    )
    t.start()
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    if agent.api_mode == "bedrock_converse":
        ...
        # Combine: ContextVars binding (upstream) + threading.local session
        # tag re-application (fork). See the non-streaming call above.
        _sid = getattr(agent, "session_id", None)
        t = threading.Thread(
            target=_context_thread_target(
                lambda: run_with_session_context(_sid, _bedrock_call)
            ),
            daemon=True,
        )
        t.start()
    ...
    # Combine: ContextVars binding (upstream) + threading.local session tag
    # re-application (fork). See the non-streaming call above.
    _sid = getattr(agent, "session_id", None)
    t = threading.Thread(
        target=_context_thread_target(
            lambda: run_with_session_context(_sid, _call)
