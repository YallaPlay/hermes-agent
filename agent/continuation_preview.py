...
_PROJECTOR_INSTRUCTION = (
    ...
    "next_gate, and uncertainties. All fields are required; use [] for empty arrays. "
    "Field shapes (all values are strings unless stated): objective is a string; "
    "acceptance_criteria, constraints, and open_questions are arrays of strings; "
    "decisions is an array of {decision, rationale, user_confirmed(bool), citation?}; "
    "blockers is an array of {blocker, unblock_condition, owner?}; "
    "dependencies is an array of {dependency, state, verification?}; "
    "approvals is an array of {scope, status(requested|approved|denied), source_ref?, citation?}; "
    "external_effects is an array of {effect, disposition(planned|in_progress|attempted_unknown|"
    "succeeded|failed|cancelled|blocked), retry_policy(do_not_retry|retry_safe|verify_first), "
    "receipt_ref?, recheck_action?, citation?} where attempted_unknown requires verify_first "
    "plus recheck_action; "
    "runtime_handles is an array of {kind(process|delegation|job|other), id, "
    "observed_state, recheck_action}; "
    "artifacts is an array of {ref, kind, observed_state?, verification?}; "
    "retry_hazards is an array of {operation, risk, safe_recovery?}; "
    "remaining_work is an array of {item, acceptance_ref?}; "
    "next_gate is one object {action, verification, expected_observation, on_success?, "
    "on_failure?, on_unknown?, citation?}; "
    "uncertainties is an array of {claim, recovery_source, why_uncertain?}. "
    "A citation is {message_id(int), quote(verbatim span from that message)}. "
    "Cite message_id and exact quotes. Assistant and tool rows "
