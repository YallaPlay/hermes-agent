...
compression:
  ...
  proactive_prune_artifacts: false                   # Persist redacted pruned result bodies in the active sandbox (see below)
...
`proactive_prune_tokens` enables a deterministic, no-LLM prune of old tool-result payloads that runs independently of `threshold`. On large-window models the `threshold` compaction (≈50% of the window) rarely fires, so bulky tool outputs (terminal dumps, file reads, web extracts) ride along in history and get re-sent on every subsequent turn. When re-sent history exceeds `proactive_prune_tokens` (default `0` = off; try `48000` to enable), the legacy mode dedupes identical results, summarizes older oversized ones, and truncates large tool-call arguments — protecting the most recent `protect_last_n` messages and never calling the model. `proactive_prune_min_result_chars` (default `8000`, clamped to ≥ 200) sets the size below which a tool result is left untouched. `proactive_prune_min_reclaim_tokens` (default `4096`) prevents a prune from committing unless it reclaims at least that many tokens — a committed prune rewrites already-sent history and invalidates the provider's prompt-cache prefix, so this gate keeps those cache breaks episodic and amortized (one meaningful break, like a compression boundary) instead of firing on every tool iteration. This runs only under the built-in `compressor` engine; other context engines inherit a no-op.

`proactive_prune_artifacts` is a strict YAML boolean (default `false`) and only has an effect when `proactive_prune_tokens` is enabled. In this mode Hermes does **not** use the legacy lossy deduplication or tool-call-argument truncation passes. It redacts each old TOOL RESULT body that the proactive summarize pass would replace, stores the sanitized UTF-8 bytes under an immutable task-scoped path containing the content digest in the active sandbox, verifies the published SHA-256, and replaces the body with a self-delimiting reference containing the path, sanitized character count, digest, redaction status, tool/call identity, and read/verification instructions. Missing task scope or sandbox, sanitizer/write/hash failure, or partial-attempt failure cancels the whole prune without a lossy fallback. The measured-reclaim gate still applies, and newly created files are removed when an attempt is rejected without affecting concurrent attempts.

Artifact references share the existing sandbox result-persistence lifecycle: they are private to the active environment and remain available only for that sandbox's lifetime. They are **not** durable across sandbox cleanup, environment teardown, or later session restoration. This option adds no separate garbage collector; each prune attempt owns its immutable files, and rejected attempts clean up only files they created.
